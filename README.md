...
All the Models and Client are generated by the supplied [Retailer API specifications by Bol](https://api.bol.com/retailer/public/apispec/v8). Generating ensures there are no typos, not every operation needs a test and future (minor) updates to the specifications can easily be applied.

The generated classes contain all data to properly map method arguments and response data to the models: the specifications are only used to generate them. To build the classes for the latest bol.com Retailer API version, download the latest Retailer API specification to `src/OpenApi/apispec.json` before generating.
